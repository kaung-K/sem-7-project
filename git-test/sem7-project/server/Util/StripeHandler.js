...
try {
  ...
  console.warn("Invoice write failed (non-blocking):", e?.message);
      ...
      try {
        await notify({
          userId: creator._id,                 // recipient: the creator
          actorId: user._id,                   // who caused it: the subscriber
          type: "sub:new",                     // your type key
          message: `${user.username || "A user"} subscribed to you`,
          metadata: {
    subscriberId: user._id,
    creatorId: creator._id,
    link: `/creator/${creator._id}`, // direct to profile
  },
        });
      } catch (nErr) {
        console.warn("notify(sub:new) failed:", nErr);
        // don't block the webhook on notification hiccups
      }
